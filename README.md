...
基於組織的資源條件，在新增權限之前先使用 `resourceOrg()`，
...
基於角色的資源條件，在新增權限之前先使用 `resourceRole()`。

該注意的是：每當你新增一次權限，你就**必須再使用一次**這個函式，**否則你下次新增的權限會不基於角色資源條件**。
...
基於種類的資源條件，在新增權限之前先使用 `resourceType()`。
